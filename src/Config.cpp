...
{
    printf("[%s] %s:%d: void Config::showHelp(FILE *f) [after]\n", __func__, __FILE__, __LINE__);
    List<String> out;
    int longest = 0;
    ...
    for (int i=0; i<count; ++i) {
        if (out.at(i).isEmpty()) {
            ...
            String description = sOptions.at(i).description;
            printf("%s => %s\n", description.constData(), sOptions.at(i).defaultValue.toString().constData());
            if (!sOptions.at(i).defaultValue.isNull()) {
                const bool dot = description.endsWith('.');
                description.insert(description.size() - dot ? 1 : 0,
                                   String::format<64>(" (default %s)", sOptions.at(i).defaultValue.toString().constData()));
            }
            fprintf(f, "%s%s %s\n",
                    out.at(i).constData(),
